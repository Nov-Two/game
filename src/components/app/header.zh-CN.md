...
## 与 Store/Mock 的协作
- 组件依赖 `state.player.uid` 与 `state.eventConfig.rule`
  - 开发态可通过 Mock 接口 `/api/info` 返回 `player.uid` 与 `event_config.rule`
  - 在应用启动阶段（如根组件）调用 `initConfig` 拉取配置后再渲染 AppHeader，确保 UID 与规则内容可用
